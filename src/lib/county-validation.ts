...
export function countyValidation(request: HttpRequest, context: InvocationContext, mail: string = null): string[] {
  ...
  if (typeof allowedUpnSuffixString !== "string") {
    ...
    throw new HTTPError(
      401,
      "Unauthorized: Invalid security key in header or query string",
      `Partially masked key: ${getPartiallyMaskedKey(securityValue)}`
    );
  }
  ...
  if (userMail && !allowedUpnSuffixes.some((suffix) => userMail.endsWith(suffix))) {
    context.error(
      `Forbidden: User mail (${userMail}) does not match any of the allowed UPN suffixes: [${allowedUpnSuffixes.join(", ")}]. Partially masked key: ${getPartiallyMaskedKey(securityValue)}`
    );
    throw new HTTPError(
      403,
